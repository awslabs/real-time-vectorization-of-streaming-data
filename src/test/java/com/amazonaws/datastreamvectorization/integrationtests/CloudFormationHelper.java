...
public class CloudFormationHelper {
    ...
    private final static String BEDROCK_VPC_ENDPOINT_OUTPUT_KEY = "BedrockVpcEndpoint";
    private final static String OPENSEARCH_ENDPOINT_OUTPUT_KEY = "OpenSearchVpcEndpoint";
    private final static List<StackStatus> TERMINAL_STACK_STATUSES = List.of(
    ...
    public String getParameterValue(Stack stack, String parameterKey) {
        List<Parameter> matchingParameters = stack.getParameters()
                .stream()
        ...
        return matchingParameters.get(0).getParameterValue();
    }
    ...
    public Stack createBlueprintStack(String templateURL,
        ...
        try {
            ...
            Stack stack = this.pollBlueprintStatus(stackName);
            if (stack == null) {
     ...
     * @return The deployed stack, or null if the stack doesn't exist during the last polling retry
     */
    private Stack pollBlueprintStatus(String stackName) {
        int retryCount = 0;
        String stackStatus;
        Stack stack = null;
        try {
            ...
            if (e.getErrorMessage().contains("does not exist")) {
                // this means a stack deletion completed successfully
                return null;
            }
            throw e;
        }
    ...
    public void deleteBlueprintStack(String stackName) {
        ...
        Stack stack = this.pollBlueprintStatus(stackName);
        if (stack == null) {
            // Stack can complete deletion causing poll status to return null, this is OK
            return;
        }
     ...
     * Returns empty string if there is no Bedrock VPCE created by the stack.
     *
    ...
    private String getBedrockVpceToDelete(String outputValue) {
        ...
        if (matcher.find()) {
            ...
            boolean vpceBoolean = Boolean.parseBoolean(matcher.group("vpceBoolean"));
            System.out.println("Bedrock VPC endpoint created by stack? -> " + vpceBoolean);
            if (vpceBoolean) {
                System.out.println("Bedrock VPC endpoint ID created by stack -> " + vpceID);
                return vpceID;
            } else {
                return "";
            }
     ...
     * Returns empty string if there is no OpenSearch VPCE created by the stack.
     *
    ...
    private String getOpenSearchVpceToDelete(String outputValue) {
        ...
        if (matcher.find()) {
            String vpceID = matcher.group("vpceID");
            boolean vpceBoolean = Boolean.parseBoolean(matcher.group("vpceBoolean"));
            System.out.println("OpenSearch VPC endpoint created by stack? -> " + vpceBoolean);
            if (vpceBoolean) {
                System.out.println("OpenSearch VPC endpoint ID created by stack -> " + vpceID);
                return vpceID;
            } else {
                return "";
            }
